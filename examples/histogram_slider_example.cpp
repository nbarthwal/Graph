...
public:
        ...
        : mean_(mean), sigma_(sigma), color_(std::move(color))
    {
        for (std::size_t bin = 0; bin < kBinCount; ++bin) {
            centers_.push_back(kMinCenter + static_cast<float>(bin) * kBinWidth);
        }
    }

    std::string Color() const override { return color_; }
    std::size_t BinCount() const override { return kBinCount; }
    float BinCenter(std::size_t bin) const override { return centers_.at(bin); }
    float BinWidth() const override { return kBinWidth; }

    float Count(float p, std::size_t bin) const override
    {
        const float x = centers_.at(bin);
        const float exponent = -((x - mean_) * (x - mean_)) / (2.0f * sigma_ * sigma_);
...
public:
    SliderHistogram()
    {
        data_sets_.push_back(std::make_unique<GaussianHistogramData>(4.0f, 1.2f, "blue"));
        data_sets_.push_back(std::make_unique<GaussianHistogramData>(7.0f, 1.5f, "red"));
        data_sets_.push_back(std::make_unique<GaussianHistogramData>(2.5f, 0.9f, "green"));
    }

    float MaxX() const override { return 12.0f; }
    float MinX() const override { return 0.0f; }
    float MaxY() const override { return 12.0f; }
    float MinY() const override { return 0.0f; }
    float MaxP() const override { return 2.0f; }
    float MinP() const override { return 0.5f; }
